...
The Playliner plugin cannot be installed in this environment, so the full `playliner-search`
skill is embedded below. When the user asks about mobile games (articles, releases, events,
monetization, analytics), follow the instructions below exactly. Take the question from the
user's most recent message. Do not answer from your own knowledge.
...
All API calls use the helper `~/.config/playliner/playliner-api.sh`. It is NOT on
`PATH`. If the file does not exist yet, create it from the listing below (run once):

```bash
mkdir -p ~/.config/playliner
cat > ~/.config/playliner/playliner-api.sh <<'PLAYLINER_API_EOF'
#!/usr/bin/env bash
#
# Thin wrapper around the Playliner /api/v1/external/* search API.
#
# Usage:
#   bash ~/.config/playliner/playliner-api.sh <endpoint> [json-body]
#
...
PLAYLINER_API_EOF
chmod +x ~/.config/playliner/playliner-api.sh
```

Then resolve its path once and reuse the variable for every call:

```bash
PLAYLINER_API="$HOME/.config/playliner/playliner-api.sh"
test -f "$PLAYLINER_API" && echo OK || echo MISSING
```

Every example below invokes the helper as `"$PLAYLINER_API"` (run
`bash "$PLAYLINER_API" …` if the file is not executable).
...
(the variable resolved in **Set up the helper script**).
